...
const char * const PIVX_CONF_FILENAME = "esher.conf";
const char * const PIVX_PID_FILENAME = "esher.pid";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\esher
// Windows >= Vista: C:\Users\Username\AppData\Roaming\esher
// Mac: ~/Library/Application Support/esher
// Unix: ~/.esher
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Esher";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\esherParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\esherParams
    // Mac: ~/Library/Application Support/esherParams
    // Unix: ~/.esher-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EsherParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "EsherParams";
#else
    // Unix
    return pathRet / ".esher-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty esher.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Esher config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override esher.conf
