...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    // premine for merge ALTC, DLN, MPTC - the over supply will be burnt
    // this premine will not be used for staking, several MN will be deployed to support the network until users starts creating their masternodes
    if (nHeight ==1)		return 190000000 * COIN;
...
{
    if (nHeight > 1000) return GetBlockValue(nHeight) * 75 / 100;
    return 0;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 ESHER tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 ESHER tx -> 1 confirmation
