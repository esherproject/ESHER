...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Esher client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Esher"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Esher"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
